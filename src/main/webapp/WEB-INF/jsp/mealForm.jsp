<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>

<html>
<jsp:include page="fragments/headTag.jsp"/>
<body>
<jsp:include page="fragments/mealHeader.jsp"/>
<section>
    <h3><a href="${pageContext.request.contextPath}/index.html">Home</a></h3>
    <hr>
    <h2>${meal.id == null ? 'Create meal' : 'Edit meal'}</h2>

    <jsp:useBean id="meal" type="ru.javawebinar.topjava.model.Meal" scope="request"/>

    <form method="post" action="${meal.id == null ? '/topjava/meals' : '/topjava/meals'}">
        <c:if test="${meal.id != null}">
            <input type="hidden" name="id" value="${meal.id}">
        </c:if>

        <dl>
            <dt>DateTime:</dt>
            <dd>
                <input type="datetime-local" value="${fn:substring(meal.dateTime, 0, 16)}" name="dateTime" required>
            </dd>
        </dl>

        <dl>
            <dt>Description:</dt>
            <dd>
                <input type="text" value="${meal.description}" size="40" name="description" required>
            </dd>
        </dl>

        <dl>
            <dt>Calories:</dt>
            <dd>
                <input type="number" value="${meal.calories}" name="calories" required>
            </dd>
        </dl>

        <button type="submit">Save</button>
        <button onclick="window.history.back()" type="button">Cancel</button>
    </form>
</section>
<jsp:include page="fragments/footer.jsp"/>
</body>
</html>
